...
import { getListsById , deleteList ,setNotification ,setListsIdToDelete} from '../store/actions';
...
const DeleteListModal: FC<DeleteListModalProps> = ({ listId }) => {
    ...
    useEffect(() => {
        dispatch(getListsById(listId))
    }, [dispatch, listId]);

    const deleteListHandler = () => {
        dispatch(deleteList(listId))
        if(list) {
            dispatch(setNotification(`List "${list.name}" delete!` , 'danger'))
        }
    }

    const hideModalHandler = () => {
        dispatch(setListsIdToDelete(''));
    }
            ...
            <div className="modal-card">
                ...
                <div className="modal-card-body">
                    ...
                    <div className="content">
                        {list?.tasks.length === 0 ? <p className="has-text-centered pt-4 mb-0">No Tasks in this list</p>
                            :
                            <ul>
                                {list?.tasks.map(task => (
